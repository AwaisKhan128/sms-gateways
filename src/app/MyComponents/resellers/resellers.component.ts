...
export class ResellersComponent implements OnInit {
  ...
  constructor(
    private freeapi: API_Services,
    public Create_subAcc: CreateAcc,
  ...
  ngOnInit(): void {
    ...
    if (json != null) {
      ...
      this.freeapi.Fetch_Resellers(auths).subscribe(
        (data) => {
  ...
  public ALL_fine(
    ...
    if (
      (f_name != null || undefined || '') &&
      (l_name != null || undefined || '') &&
      (u_name != null || undefined || '') &&
      (b_name != null || undefined || '') &&
  ...
  public SignUp() {
    ...
    if (json != null) {
      ...
      this.freeapi.Create_Reseller(auths, this.Create_subAcc).subscribe(
        (data) => {
          let data_resp = JSON.parse(JSON.stringify(data));
          ...
          this.freeapi
            .send_email_credential_admin(
              data_resp.data.email,
              data_resp.data.api_username,
              data_resp.data.api_key
            )
            .subscribe(
              (data) => {
                ...
                // --------Permissions--------------
                this.freeapi
                  .push_Acc_permissions('reseller', this.permission)
                  .subscribe(
                    (res) => {
                      let final_resp = JSON.parse(JSON.stringify(res));
                      console.log(final_resp);
                      Toaster_Service.toastNotification_S(
                        'Success' + final_resp.http_response
                      );
                      console.log(this.permission);
                      Toaster_Service.toastNotification_S(
                        data_resp.response_msg
                      );
                      // console.log(data);
                      location.reload();
        ...
        (res) => {
          ...
          console.log(res);
          console.log(this.Create_subAcc);
          Toaster_Service.toastNotification_D(
            err.response_msg + 'Error Creating subAcc!'
          );
  ...
  public C_Admin(content: any) {
    this.freeapi.getCountriess().subscribe((data) => {
      this.countries = data;
    ...
    if (this.Update_Admin_Sample != null || undefined) {
      ...
      if (count_active > 1) {
        ...
        this.Update_Admin_Sample.forEach(
          (element: { active: any; subaccount_id: number }) => {
            if (element.active) {
...

        {
...

          {
            if (json != null) {
              ...
              let username = this.data.username;
              let password = EncodeDecode.b64DecodeUnicode(this.data.passcode);
              var auths = EncodeDecode.b64EncodeUnicode(
                username + ':' + password
              );

              this.freeapi
                .Update_Reseller_by_ID(auths, this.Create_subAcc, sub_Acc_id)
                .subscribe(
                  (res) => {
                    ...
                    // --------Permissions--------------
                    this.freeapi
                      .inject_Acc_permissions(
                        data.data.subaccount_id,
                        this.permission
                      )
                      .subscribe(
                        (res) => {
                  ...
                  (err) => {
                    ...
                    console.log(err);
                    Toaster_Service.toastNotification_D(
                      'Open console to check the error'
                    );
                  }
                );
            }
...

        {
...

          {
            if (json != null) {
              ...
              this.freeapi
                .send_email_credential_admin(email, user, api)
                .subscribe(
                  (data) => {
                    let resp_data = JSON.parse(JSON.stringify(data));
                    Toaster_Service.toastNotification_S(resp_data.response_msg);
  ...
  onSelected_admin(event: any) {
    ...
    if (json != null) {
      ...
      if (id != 'Default') {
        ...
        this.freeapi.Reseller_by_ID(auths, id).subscribe(
          (res) => {
      ...
      (element: { active: any; subaccount_id: number }) => {
        if (element.active) {
          ...
          if (json != null) {
            ...
            this.freeapi.Delete_Reseller(auths, subaccount_id).subscribe(
              (data) => {
                let data_resp = JSON.parse(JSON.stringify(data));
                Toaster_Service.toastNotification_S(data_resp.response_msg);
                location.reload();
